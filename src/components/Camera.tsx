    ...
    if (selectedMember) {
      ...
      img.onload = () => {
        ...
        captureImg.onload = () => {
          ...
          ctx?.drawImage(
            img,
            canvas.width - memberWidth - padding,
            canvas.height - memberHeight - padding,
            memberWidth,
            memberHeight
          );
  ...
  const share = async () => {
    ...
    try {
      ...
      await navigator.share({
        files: [
          new File([blob], 'xg-camera.jpg', { type: 'image/jpeg' })
        ]
      });
    } catch (error) {
      ...
      {!imgSrc ? (
        <>
          <div className="relative w-full aspect-[9/16] bg-black rounded-lg overflow-hidden">
            ...
            {selectedMember && (
              <div className="absolute bottom-20 right-4 w-1/3 h-auto flex items-center justify-center">
                <img
